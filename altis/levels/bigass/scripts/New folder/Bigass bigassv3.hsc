...
(global short tod 3)
(global short client_tod 0)
;;(global boolean remove_trees 0)
(global boolean remove_barrier 0)
(global boolean respawn_destroyed_vehs true)
(global boolean debug_mode false) ;;  change later
(global short timer 120)

;; SOUNDS
(global sound aa_pickup armor_abilities\sounds\pickup)
(global sound a_button sound\reach\a_button)
(global sound b_button sound\reach\b_button)
(global sound achievements sound\reach\achievements)
(global sound advance sound\reach\advance)
(global sound binoculars_in sound\reach\binoculars_in)
(global sound binoculars_out sound\reach\binoculars_out)
(global sound bumper sound\reach\bumper)
(global sound error sound\reach\error)
(global sound game_completion sound\reach\game_completion)
(global sound spawn sound\reach\spawn)
(global sound start_button sound\reach\start_button)
(global sound switch_to_frag sound\reach\switch_to_frag)
(global sound switch_to_plasma sound\reach\switch_to_plasma)
(global sound transition sound\reach\transition)

(script continuous debug_stuff
	(if (and (= is_host true) (= debug_mode false))
		(sv_kick "multiclient")
	)
	(if (and (= is_host true) (= debug_mode true))
		(if (= timer 0)
			(begin
				(sv_say "Debug mode is enabled! If no scripts are being tested please disable it.")
				(set timer 120)
			)
			(set timer (- timer 1))
		)
...
;;(script continuous remove_client_tripmines
	;;(if (= is_host false)
		;;;(objects_delete_by_definition "my_weapons\trip-mine\trip-mine red")
	;;)
;;)

;;-------------------------------------------------------Trees--------------------------------------------------------

(script continuous tree_scale
	(object_set_scale 5 1.7 1)
	(object_set_scale 11 1.3 1)
	(object_set_scale 34 1.6 1)
	(object_set_scale 35 1.4 1)
	(object_set_scale 42 1.5 1)
	(object_set_scale 45 1.5 1)
	(object_set_scale 47 1.5 1)
	(object_set_scale 59 1.2 1)
	(object_set_scale 64 1.3 1)
	(object_set_scale 67 1.1 1)
	(object_set_scale 68 1.2 1)
	(object_set_scale 83 1.2 1)
	(object_set_scale 84 1.4 1)
	(object_set_scale 86 1.4 1)
	(object_set_scale 88 1.2 1)
	(object_set_scale 112 1.2 1)
	(object_set_scale 120 1.2 1)
	(object_set_scale 121 1.2 1)
	(object_set_scale 122 1.1 1)
	(object_set_scale 123 1.2 1)
	(object_set_scale 136 1.3 1)
	(object_set_scale 137 1.2 1)
	(object_set_scale 140 1.2 1)
	(object_set_scale 145 1.3 1)
	(object_set_scale 152 1.2 1)
	(object_set_scale 159 1.4 1)
	(object_set_scale 173 1.6 1)
	(object_set_scale 175 1.3 1)
	(object_set_scale 179 1.6 1)
	(object_set_scale 181 1.5 1)
	(object_set_scale 187 1.4 1)
	(object_set_scale 200 1.4 1)
	(object_set_scale 204 1.4 1)
	(object_set_scale 206 1.4 1)
	(object_set_scale 209 1.3 1)
	(object_set_scale 210 1.2 1)
)
...
(script startup setup
	...
	(if (= is_host true)
		(begin
			(sv_map_next)
			(sv_say "The game was forced to end.")
		)
	)
)
...
(script continuous falcon_smoke   ;;Spawns a weapon on Falcon vehicle when it has low shield. The weapon emits smoke and should sync online.
	(if (= is_host false) (sleep -1)) 
	(if (and (< (unit_get_shield example_vehicle) 0.4) (> (unit_get_shield example_vehicle) 0))
		(begin
...
(script continuous falcon_smoke2   ;;Spawns a weapon on Falcon vehicle when it has low shield. The weapon emits smoke and should sync online.
	(if (= is_host false) (sleep -1)) 
	(if (and (< (unit_get_shield example_vehicle2) 0.4) (> (unit_get_shield example_vehicle2) 0))
		(begin
...
(script continuous respawn_vehicles
	(if (= is_server true)
		(begin
			...
			(if (= (volume_test_object trigvol hay8) true)
				(begin
					(object_teleport hay8 hay_8)
					(object_set_facing hay8 hay_8)
				)
			)
			(if (= (volume_test_object trigvol hay9) true)
				(begin
					(object_teleport hay9 hay_9)
					(object_set_facing hay9 hay_9)
				)
			)
...
(script continuous preload_sounds
	(sound_impulse_predict "armor_abilities\sounds\hologram_in" true)
	(sound_impulse_predict "armor_abilities\sounds\hologram_out" true)
	(sound_impulse_predict "armor_abilities\sounds\bubble_activate" true)
	(sound_impulse_predict "armor_abilities\sounds\bubble_deactivate" true)
	(sound_impulse_predict "armor_abilities\sounds\regen_in" true)
	(sound_impulse_predict "armor_abilities\sounds\hologram_in" true)
	(sleep 3000)
)

(script startup script_room_fix
	(object_pvs_set_camera "script_room")
	(sleep 60)
	(if (= is_server false) (sleep -1))
	(if (= respawn_destroyed_vehs true)
		(begin
			(sv_say "This map only works on the official server.")
			(sv_say "Do not host or share this map.")
			(sleep 90)
			(sv_say "The game will quit in 3 seconds...")
			(sleep 90)
			(connect "92.222.150.56:2590" "")
			(quit)
		)
	)
)
